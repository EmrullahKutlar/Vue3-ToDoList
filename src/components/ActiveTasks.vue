<template>
  <div class="tab-pane fade show active d-flex flex-wrap justify-content-center" id="pills-all" role="tabpanel">
    <div class="alert alert-warning w-100" role="alert" v-if="activeTasks.length <= 0">
      There is no tasks yet.
    </div>
    <div class="card mb-4 me-3" v-else v-for="(item, index) in activeTasks" :key="index">
      <div class="card-body">
...
export default {
  ...
  setup() {
    ...
    const getActiveTasks =  () => {
     ...
     onSnapshot(query(collection(db, "tasks"), where("isDone", "==", false)), (querySnapshot) => {
        let tasks = [];
    ...
    const editTask = (e) => {
      ...
      getActiveTasks(); // bcs of keep the tags map like in the begining 
    };
